...
int main(void) {
  ...
  while (true) {
      ...
      if (is_at_start(mouse.pos)) {
        ...
        if (mouse.path_length > 0) { // Only verify if a path was actually found
          ...
          if (verify_path_exploration(&mouse, &maze)) {
            ...
            log_message(
                "=== Path requires exploration! Returning to SEARCH_MODE ===");
            ...
            for (int i = 0; i < mouse.path_length; ++i) {
              ...
              if (!maze.visited[p.x][p.y]) {
                ...
                snprintf(buffer, sizeof(buffer),
                         "Targeting first unvisited cell on path: (%d,%d)",
                         target_unvisited.x, target_unvisited.y);
                log_message(buffer);
                break;
              }
            }
            ...
            // The main loop will now use the new distances to explore towards
            // the target
          }
        } else {
          log_message("ERROR: No path computed after returning to start. "
                      "Cannot proceed.");
          // Handle error: maybe try flooding to goal and searching again? Or
          // stop. For now, just stay put or enter a safe stop state. Consider
          // adding a specific ERROR_MODE? As a fallback, maybe just try
          // searching again:
    ...
    char buffer[100];
    snprintf(buffer, sizeof(buffer),
             "State: Pos=(%d,%d) Orient=%d Mode=%d GoalFound=%d", mouse.pos.x,
             mouse.pos.y, mouse.orientation, mouse.mode, mouse.goal_found);
    log_message(buffer);
  }
  ...
  for (int x = 0; x < MAZE_WIDTH; x++) {
    for (int y = 0; y < MAZE_HEIGHT; y++) {
      ...
      for (Direction dir = 0; dir < DIRECTION_COUNT; dir++) {
        m->walls[x][y][dir] =
            false; // Assume no walls initially (except boundaries)
      }
    }
  }
...
void init_mouse(MouseState *ms, Maze *m) {
  ...
  ms->goal_found = false;
  ms->path_length = 0;
  m->visited[0][0] = true; // Mark starting cell visited
}
...
void set_wall(Maze *m, Point p, Direction dir) {
  ...
  Point neighbor_pos = {p.x + direction_delta[dir].x,
...
void flood_fill(Maze *m, Point target) {
  ...
  while (q_head < q_tail) {
    ...
    for (Direction dir = 0; dir < DIRECTION_COUNT; dir++) {
      ...
      Point neighbor = {current.x + direction_delta[dir].x,
...
// Prefers lower distance values. In SEARCH_MODE, adds a small bias towards
// unvisited cells.
Direction choose_next_direction(const MouseState *ms, const Maze *m) {
  Point current_pos = ms->pos;
  int   min_dist = INVALID_DISTANCE +
                 10; // Initialize higher than max possible distance + bonus
  ...
  for (Direction dir = 0; dir < DIRECTION_COUNT; dir++) {
    ...
    Point neighbor = {current_pos.x + direction_delta[dir].x,
                      current_pos.y + direction_delta[dir].y};
    ...
    if (ms->mode == SEARCH_MODE && !m->visited[neighbor.x][neighbor.y]) {
      // Make unvisited significantly more attractive than visited cells with
      // the *same* base distance. If an unvisited cell has a higher base
      // distance, we still prefer lower distance overall.
      adjusted_dist -= 1; // Simple bonus - adjust magnitude as needed
    }

    // If this neighbor has a lower (potentially adjusted) distance, it's the
    // new best
  ...
  if (!found_move) {
    // This should ideally not happen if flood fill is correct and there's a
    // path
...
// Computes the shortest path from start (0,0) to the goal area using the
// current maze map
void compute_shortest_path(MouseState *ms, Maze *m) {
  ...
  while (!is_at_goal(current_pos)) {
    ...
    for (Direction dir = 0; dir < DIRECTION_COUNT; dir++) {
      ...
      if (is_within_bounds(neighbor) &&
          m->distances[neighbor.x][neighbor.y] < min_dist) {
    ...
    if (!found_next) {
      log_message("ERROR: Could not find next step while computing shortest "
                  "path! Path broken?");
    ...
    // char buffer[50];
    // sprintf(buffer, "Path point %d: (%d,%d)", ms->path_length - 1,
    // current_pos.x, current_pos.y); log_message(buffer);
  }

  char buffer[80];
  snprintf(buffer, sizeof(buffer),
           "Shortest path computed with %d steps (length %d including start).",
           ms->path_length - 1, ms->path_length);
  log_message(buffer);
}
...
bool verify_path_exploration(const MouseState *ms, const Maze *m) {
  ...
  for (int i = 0; i < ms->path_length; ++i) {
    ...
    if (!is_within_bounds(p)) {
      char buffer[100];
      snprintf(buffer, sizeof(buffer),
               "Path verification FAILED: Point %d (%d,%d) is out of bounds.",
               i, p.x, p.y);
      log_message(buffer);
    ...
    if (!m->visited[p.x][p.y]) {
      ...
      snprintf(
          ...
          "Path verification FAILED: Point %d (%d,%d) on path was not visited.",
          i, p.x, p.y);
      log_message(buffer);
      return false; // Found an unvisited cell on the path
    }
    // Optional stricter check: Ensure the transition between path[i-1] and
    // path[i] is known open. compute_shortest_path should already handle this
    // via has_wall, but double-checking adds robustness.
    if (i > 0) {
      ...
      for (Direction d = 0; d < DIRECTION_COUNT; ++d) {
        if (prev_p.x + direction_delta[d].x == p.x &&
            prev_p.y + direction_delta[d].y == p.y) {
          move_dir = d;
          dir_found = true;
          break;
        }
      ...
      if (!dir_found || has_wall(m, prev_p, move_dir)) {
        char buffer[120];
        snprintf(buffer, sizeof(buffer),
                 "Path verification FAILED: Transition from (%d,%d) to (%d,%d) "
                 "uses unknown/walled path segment.",
...
void follow_shortest_path(MouseState *ms, Maze *m) {
  ...
  for (int i = 1; i < ms->path_length; i++) {
    ...
    if (!dir_found) {
      char buffer[100];
      snprintf(buffer, sizeof(buffer),
               "ERROR: Speed run path invalid. Cannot determine move direction "
               "from (%d,%d) to (%d,%d)",
    ...
    if (API_moveForward()) {
      ...
      // This indicates a major inconsistency between the computed path and
      // reality
      char buffer[120];
      snprintf(buffer, sizeof(buffer),
               "FATAL ERROR: Speed run failed! Hit unexpected wall moving from "
               "(%d,%d) towards (%d,%d) facing %d. Map is wrong!",
               ms->shortest_path[i - 1].x,
               ms->shortest_path[i - 1].y, // Log previous position
    ...
    // Log progress
    char buffer[80];
    snprintf(buffer, sizeof(buffer), "Speed run: Reached (%d,%d)", ms->pos.x,
             ms->pos.y);
    log_message(buffer);
  }
  ...
  for (int x = 0; x < MAZE_WIDTH; x++) {
    for (int y = 0; y < MAZE_HEIGHT; y++) {
      ...
      if (m->distances[x][y] == INVALID_DISTANCE) {
        ...
        char buffer[8];
        snprintf(buffer, sizeof(buffer), "%d", m->distances[x][y]);
      ...
      if (ms->mode == SPEED_MODE && ms->path_length > 0) {
        ...
        if (on_path && !(p.x == ms->pos.x && p.y == ms->pos.y) &&
            !is_at_goal(p)) {
